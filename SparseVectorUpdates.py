...
#math.log(x/ y), 10)
    def update_tfidf_score_dictionaries(self,number_of_documents):
        ...
        for word, doc_id in self.relevant_term_frequency_dict.keys():
                ...
                idf = IDF_SMOOTHING_CONSTANT + math.log(number_of_documents/self.document_frequency_dict[word])
                tf_idf_score = tf*idf
    ...
    def update_query_vector_rocchios_algorithm(self, num_relevant_docs, num_non_relevant_docs, alpha=0.5, beta =0.5, gamma = 0.75):
        ...
        ROCCHIO_DENOMINATOR_SMOOTHING_CONSTANT = 0

        num_non_relevant_docs = ROCCHIO_DENOMINATOR_SMOOTHING_CONSTANT + num_non_relevant_docs
        num_relevant_docs = ROCCHIO_DENOMINATOR_SMOOTHING_CONSTANT + num_relevant_docs
